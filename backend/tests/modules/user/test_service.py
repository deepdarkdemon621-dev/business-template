...
async def test_create_user_hashes_password_and_sets_flags(
    db_session: AsyncSession, actor: User, db_audit_ctx
) -> None:
...
async def test_update_user_applies_partial(
    db_session: AsyncSession, actor: User, db_audit_ctx
) -> None:
...
async def test_soft_delete_user_flips_is_active(
    db_session: AsyncSession, actor: User, db_audit_ctx
) -> None:
...
async def test_soft_delete_blocked_when_self(
    db_session: AsyncSession, actor: User, db_audit_ctx
) -> None:
...
async def test_superadmin_can_self_delete(
    db_session: AsyncSession, superadmin_actor: User, db_audit_ctx
) -> None:
...
async def test_revoke_role_removes_row(
    db_session: AsyncSession, actor: User, db_audit_ctx
) -> None:
